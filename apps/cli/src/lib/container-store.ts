...
export function findContainersByWorkspace(config: ConfigFile, workspace: string): ContainerRecord[] {
    return Object.values(config.containers).filter((c) => c.workspace === workspace && c.removedAt === null);
}
...
export function updateContainer(config: ConfigFile, id: string, patch: Partial<ContainerRecord>): void {
 ...
 */
export function syncContainerStatuses(config: ConfigFile, dockerStates: Map<string, ContainerStatus>): void {
