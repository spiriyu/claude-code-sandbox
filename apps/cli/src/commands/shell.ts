...
export function makeShellCommand(): Command {
    return new Command('shell').description('Open a new bash session inside the container (exit closes the session, not the container)').action(async function (this: Command) {
