...
export function makeStartAllCommand(): Command {
    return new Command('start-all').description('Start all stopped containers (does not create new ones)').action(async function (this: Command) {
        ...
        const toStart = getAllContainers(config, false).filter((c) => c.lastStatus !== 'running' && c.lastStatus !== 'paused');
